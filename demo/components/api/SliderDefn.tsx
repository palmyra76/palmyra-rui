...
import { sliderProps, sliderPropsDes } from "./options/Validation";
import { eventListener, eventListenerDes, valueListener, valueListenerDes } from "./options/listener";

const SliderDefn = () => {

    const SliderImport = `import { MuiSlider } from "palmyra/MuiSlider"`
    ...
    const data: GridProperties = [
        ...
        { property: 'fieldProps', type: `'small'`, description: `The size of the input element.` },
        ...
        { property: 'minDistance', type: 'number', description: 'The minDistance is like a small gap you need to move past before the slider reacts and starts to move.' },
        { property: 'range', type: 'boolean', description: 'The range is set to minimum and maximum values for slider.' },
        { property: 'sliderProps', type: sliderProps(), description: sliderPropsDes() },
    ]

    return (
        <div>
            <Grid data={data} header="Slider API" import={SliderImport} />
        </div>
    )

}

export default SliderDefn;