    ...
    return g = { 705: (i) => {
      i.exports = function(t, r) {
        ...
        return r.length === 0 ? n.push({ text: t, highlight: !1 }) : r[0][0] > 0 && n.push({ text: t.slice(0, r[0][0]), highlight: !1 }), r.forEach((p, s) => {
          const a = p[0], h = p[1];
          n.push({ text: t.slice(a, h), highlight: !0 }), s === r.length - 1 ? h < t.length && n.push({ text: t.slice(h, t.length), highlight: !1 }) : h < r[s + 1][0] && n.push({ text: t.slice(h, r[s + 1][0]), highlight: !1 });
        }), n;
      };
    } }, o = {}, function i(t) {
      var r = o[t];
...
(function(b, k) {
  (function(g, o) {
    ...
    return g = { 772: (i, t, r) => {
      const n = r(826).remove, p = /[.*+?^${}()|[\]\\]/g, s = /[a-z0-9_]/i, a = /\s+/;
      i.exports = function(h, O, m) {
        var y, x;
        x = { insideWords: !1, findAllOccurrences: !1, requireMatchAll: !1 }, y = (y = m) || {}, Object.keys(y).forEach((u) => {
          x[u] = !!y[u];
        }), m = x;
        const U = Array.from(h).map((u) => n(u));
        let A = U.join("");
        return (O = n(O)).trim().split(a).filter((u) => u.length > 0).reduce((u, E) => {
          const f = E.length, C = !m.insideWords && s.test(E[0]) ? "\\b" : "", j = new RegExp(C + E.replace(p, "\\$&"), "i");
          let v, l;
          if (v = j.exec(A), m.requireMatchAll && v === null) return A = "", [];
          for (; v; ) {
            l = v.index;
            const w = f - U.slice(l, l + f).join("").length, I = l - U.slice(0, l).join("").length, S = [l + I, l + f + I + w];
            if (S[0] !== S[1] && u.push(S), A = A.slice(0, l) + new Array(f + 1).join(" ") + A.slice(l + f), !m.findAllOccurrences) break;
            v = j.exec(A);
          }
          return u;
        }, []).sort((u, E) => u[0] - E[0]);
...
var ae = Y.exports;
const ue = /* @__PURE__ */ P(ae), me = V(function(k, g) {
  const { fieldDef: o, store: i, value: t, displayValue: r } = k, n = o.lookupOptions || {}, p = n.idAttribute || "id", s = n.displayAttribute || "name", a = B(null), h = s, [O, m] = R([]), [y] = R(r), [x, U] = R("");
  var A = t || "";
  const [u, E] = R(A), f = q(p) ? (e) => T(p, e) : (e) => e[p], C = q(s) ? (e) => T(s, e) : (e) => e[s];
  H(() => {
    var e = j();
    e && (m([e]), a.current = e);
  }, [y]);
  ...
  const S = () => O.length > 1, z = (e, d) => {
    const M = ue(e, d, { insideWords: !0 }), G = ie(e, M);
